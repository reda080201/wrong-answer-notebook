...
impl McpBridgeManager {
    pub fn new(store: Arc<NotebookStore>, data_dir: PathBuf) -> Self {
        Self { store, data_dir, active_context: Arc::new(Mutex::new(ActiveContext::default())),
            ...
            pairing_codes: Arc::new(Mutex::new(HashMap::new())), task: Mutex::new(None) }
    }
    ...
    pub fn create_pairing_code(&self) -> Result<String, String> {
        ...
        codes.retain(|_, expires| *expires > Instant::now()); codes.insert(code.clone(), Instant::now() + PAIRING_TTL);
        Ok(code)
    }
    pub fn rotate_token(&self) -> Result<(), String> {
        ...
        *self.auth_token.lock().map_err(|_| "MCP 인증 상태를 잠글 수 없습니다.".to_owned())? = token;
        self.pairing_codes.lock().map_err(|_| "페어링 상태를 잠글 수 없습니다.".to_owned())?.clear(); Ok(())
    }
    ...
    pub async fn test(&self) -> Result<McpBridgeStatus, String> {
        ...
        let init = client.post(&url).bearer_auth(&token).json(&json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26"}})).send().await.map_err(|e| format!("initialize 요청 실패: {e}"))?;
        if !init.status().is_success() { return Err(format!("initialize 응답 실패: {}", init.status())); }
        let initialized = client.post(&url).bearer_auth(&token).json(&json!({"jsonrpc":"2.0","method":"notifications/initialized","params":{}})).send().await.map_err(|e| format!("initialized 알림 실패: {e}"))?;
        ...
        let tools = client.post(&url).bearer_auth(&token).json(&json!({"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}})).send().await.map_err(|e| format!("tools/list 요청 실패: {e}"))?.json::<Value>().await.map_err(|e| e.to_string())?;
        if tools.pointer("/result/tools").and_then(Value::as_array).map_or(0, Vec::len) != 5 { return Err("tools/list 응답을 검증하지 못했습니다.".into()); }
        let health = client.post(&url).bearer_auth(&token).json(&json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"health_check","arguments":{}}})).send().await.map_err(|e| format!("health_check 요청 실패: {e}"))?.json::<Value>().await.map_err(|e| e.to_string())?;
    ...
    async fn start(&self, port: u16) -> Result<(), String> {
        ...
        *self.auth_token.lock().map_err(|_| "MCP 인증 상태를 잠글 수 없습니다.".to_owned())? = token;
        let state = BridgeHttpState { store: Arc::clone(&self.store), images_path: self.store.images_path().to_path_buf(), auth_token: Arc::clone(&self.auth_token), pairing_codes: Arc::clone(&self.pairing_codes), active_context: Arc::clone(&self.active_context), status: Arc::clone(&self.status), audit_path: self.data_dir.join("mcp-audit.jsonl") };
        let app = router(state);
        let new_task = tauri::async_runtime::spawn(async move { if let Err(error) = axum::serve(listener, app).await { eprintln!("local MCP bridge stopped: {error}"); } });
        if let Ok(mut task) = self.task.lock() { if let Some(old) = task.replace(new_task) { old.abort(); } }
        if let Ok(mut status) = self.status.lock() { *status = McpBridgeStatus { enabled: true, state: "running".into(), host: "127.0.0.1", port, read_only: true, bridge_version: MCP_BRIDGE_VERSION, last_connected_at: None, last_error: None, has_auth_token: true }; }
        Ok(())
    }
...
fn stopped_status() -> McpBridgeStatus { McpBridgeStatus { enabled:false, state:"stopped".into(), host:"127.0.0.1", port:DEFAULT_MCP_PORT, read_only:true, bridge_version:MCP_BRIDGE_VERSION, last_connected_at:None, last_error:None, has_auth_token:false } }
...
#[derive(Clone)]
struct BridgeHttpState { store: Arc<NotebookStore>, images_path: PathBuf, auth_token: Arc<Mutex<String>>, pairing_codes: Arc<Mutex<HashMap<String, Instant>>>, active_context: Arc<Mutex<ActiveContext>>, status: Arc<Mutex<McpBridgeStatus>>, audit_path: PathBuf }
...
async fn redeem_pairing(State(state): State<BridgeHttpState>, headers: HeaderMap, Json(body): Json<Value>) -> Response {
    if !origin_allowed(&headers) { return (StatusCode::FORBIDDEN, "허용되지 않은 Origin입니다.").into_response(); }
    let code=body.get("code").and_then(Value::as_str).unwrap_or_default(); let valid=state.pairing_codes.lock().ok().and_then(|mut codes| { codes.retain(|_, expires| *expires > Instant::now()); codes.remove(code) }).is_some();
    if !valid { return (StatusCode::UNAUTHORIZED,"유효하지 않거나 만료된 페어링 코드입니다.").into_response(); }
    let token=state.auth_token.lock().map(|value| value.clone()).unwrap_or_default(); Json(json!({"accessToken":token,"tokenType":"Bearer"})).into_response()
}
