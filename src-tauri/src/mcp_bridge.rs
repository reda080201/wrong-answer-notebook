...
impl McpBridgeManager {
    ...
    pub async fn test(&self) -> Result<McpBridgeStatus, String> {
        ...
        if tools.pointer("/result/tools").and_then(Value::as_array).map_or(0, Vec::len) != 8 { return Err("tools/list 응답을 검증하지 못했습니다.".into()); }
...
fn tool_definitions()->Vec<Value>{vec![
 ...
 tool("get_active_exam_question","현재 응시 중인 모의고사 문항을 읽습니다. 응시 중에는 정답과 해설을 반환하지 않습니다.",schema(json!({}),&[])),
...
fn active_exam_session(state:&BridgeHttpState)->Result<Option<Value>,(i32,String)>{let sessions=load_exam_sessions(state)?;let context=if state.active_exam_context_path.exists(){fs::read(&state.active_exam_context_path).ok().and_then(|bytes|serde_json::from_slice::<Value>(&bytes).ok())}else{None};let wanted=context.as_ref().and_then(|value|value.get("sessionId")).and_then(Value::as_str);let by_context=wanted.and_then(|id|sessions.iter().find(|session|session.get("id").and_then(Value::as_str)==Some(id)).cloned());Ok(by_context.or_else(||sessions.into_iter().find(|session|session.get("status").and_then(Value::as_str)==Some("in_progress"))))}
fn active_exam_question_payload(state:&BridgeHttpState)->Result<Value,(i32,String)>{let Some(session)=active_exam_session(state)? else{return Ok(json!({"active":false,"message":"현재 응시 중인 모의고사 세션이 없습니다."}));};let context=if state.active_exam_context_path.exists(){fs::read(&state.active_exam_context_path).ok().and_then(|bytes|serde_json::from_slice::<Value>(&bytes).ok())}else{None};let index=context.as_ref().and_then(|value|value.get("questionIndex")).and_then(Value::as_u64).or_else(||session.get("currentQuestionIndex").and_then(Value::as_u64)).unwrap_or(0) as usize;let question=session.get("questions").and_then(Value::as_array).and_then(|items|items.get(index)).cloned().unwrap_or(Value::Null);let submitted=session.get("status").and_then(Value::as_str)==Some("submitted");let mut safe=question.clone();if !submitted {if let Some(object)=safe.as_object_mut(){object.remove("correctAnswer");object.remove("explanation");object.remove("answerKey");}}let response=session.get("responses").and_then(Value::as_array).and_then(|items|{let number=question.get("questionNumber").and_then(Value::as_str)?;items.iter().find(|item|item.get("questionNumber").and_then(Value::as_str)==Some(number))});let images=exam_image_resources(state,&session,&question);Ok(json!({"active":true,"sessionId":session.get("id"),"title":session.get("title"),"subject":session.get("subject"),"status":session.get("status"),"questionIndex":index,"totalQuestions":session.get("questions").and_then(Value::as_array).map_or(0,Vec::len),"question":safe,"images":images,"response":response.and_then(|item|item.get("response")).cloned().unwrap_or(Value::String(String::new())),"scratchNote":response.and_then(|item|item.get("scratchNote")).cloned().unwrap_or(Value::String(String::new())),"markedForReview":response.and_then(|item|item.get("markedForReview")).and_then(Value::as_bool).unwrap_or(false),"submitted":submitted,"answerAvailable":submitted}))}
fn exam_image_resources(state:&BridgeHttpState,session:&Value,question:&Value)->Value{let Some(session_id)=session.get("id").and_then(Value::as_str)else{return Value::Array(Vec::new())};let number=question.get("questionNumber").and_then(Value::as_str).unwrap_or("");let mut names=question.get("questionImages").and_then(Value::as_array).cloned().unwrap_or_default().into_iter().filter_map(|item|item.as_str().map(str::to_owned)).collect::<Vec<_>>();if let Some(figures)=question.get("figures").and_then(Value::as_array){names.extend(figures.iter().filter_map(|item|item.get("image").and_then(Value::as_str).map(str::to_owned)));}names.sort();names.dedup();Value::Array(names.into_iter().filter_map(|filename|{if namesafe_image(&state.images_path,&filename).ok()?{Some(json!({"uri":format!("notebook-exam-image://session/{session_id}/{number}/{filename}"),"name":filename,"mimeType":mime_for(&filename).ok()?}))}else{None}}).take(MAX_RESOURCE_IMAGES).collect())}
...
mod tests {
    ...
    async fn actual_streamable_http_round_trip_enforces_protocol_and_is_read_only() {
        ...
        let tools=client.post(&url).bearer_auth("test-token").json(&request(Some(2),"tools/list",json!({}))).send().await.unwrap().json::<Value>().await.unwrap(); let tools=tools["result"]["tools"].as_array().unwrap(); assert_eq!(tools.len(),8); assert!(tools.iter().all(|tool|tool.get("inputSchema").is_some()&&tool["annotations"]["readOnlyHint"]==true));
