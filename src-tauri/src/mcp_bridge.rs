...
impl McpBridgeManager {
    ...
    pub async fn test(&self) -> Result<McpBridgeStatus, String> {
        ...
        if tools.pointer("/result/tools").and_then(Value::as_array).map_or(0, Vec::len) != 9 { return Err("tools/list 응답을 검증하지 못했습니다.".into()); }
    ...
    async fn start(&self, port: u16) -> Result<(), String> {
        ...
        *self.auth_token.lock().map_err(|_| "MCP 인증 상태를 잠글 수 없습니다.".to_owned())? = token;
        let state = BridgeHttpState { store: Arc::clone(&self.store), images_path: self.store.images_path().to_path_buf(), exam_sessions_path: self.data_dir.join("exam-sessions.json"), active_exam_context_path: self.data_dir.join("active-exam-context.json"), active_export_context_path: self.data_dir.join("active-export-context.json"), auth_token: Arc::clone(&self.auth_token), pairing_codes: Arc::clone(&self.pairing_codes), sessions: Arc::clone(&self.sessions), pairing_attempts: Arc::clone(&self.pairing_attempts), active_context: Arc::clone(&self.active_context), status: Arc::clone(&self.status), audit_path: self.data_dir.join("mcp-audit.jsonl") };
...
fn authorize(state:&BridgeHttpState, headers:&HeaderMap)->Result<(),Response> {
    ...
    let authorized = if self_test {
        state.auth_token.lock().map(|value| value.as_str() == token).unwrap_or(false)
    } else {
...
fn call_tool(state:&BridgeHttpState,params:&Value)->Result<(Value,usize),(i32,String)>{let name=params.get("name").and_then(Value::as_str).ok_or((-32602,"도구 이름이 필요합니다.".to_owned()))?;let args=params.get("arguments").cloned().unwrap_or_else(||json!({}));let payload=match name {"health_check"=>json!({"ok":true,"readOnly":true,"entryCount":state.store.load_entries().map_err(store_error)?.len(),"version":MCP_BRIDGE_VERSION}),"search_notebook"=>search_payload(state,&args)?,"get_entry"=>entry_payload(state,&args)?,"get_question"=>question_payload(state,&args)?,"get_active_question"=>active_question_payload(state,&args)?,"get_active_exam_question"=>active_exam_question_payload(state,&args)?,"get_exam_session_summary"=>exam_session_summary_payload(state,&args)?,"get_submitted_exam_result"=>submitted_exam_result_payload(state,&args)?,"get_export_context"=>export_context_payload(state,&args)?,_=>return Err((-32601,"읽기 전용 MCP 도구만 사용할 수 없습니다.".into()))};let count=payload.get("items").and_then(Value::as_array).map_or_else(||payload.get("questions").and_then(Value::as_array).map_or(1,Vec::len),Vec::len);Ok((json!({"content":[{"type":"text","text":serde_json::to_string(&payload).map_err(|_|(-32603,"응답을 만들지 못했습니다.".to_owned()))?}]}),count))}
...
fn load_active_export_context(state:&BridgeHttpState)->Option<Value>{fs::read(&state.active_export_context_path).ok().and_then(|bytes|serde_json::from_slice::<Value>(&bytes).ok())}
fn export_image_entries(state:&BridgeHttpState,entry_id:&str,filenames:&[String])->Value{let mut total=0u64;Value::Array(filenames.iter().filter_map(|filename|{if namesafe_image(&state.images_path,filename).ok()?{let size=fs::metadata(state.images_path.join(filename)).ok()?.len();if total.checked_add(size)? > MAX_RESOURCE_BYTES{return None;}total+=size;Some(json!({"uri":resource_uri(entry_id,filename),"name":filename,"mimeType":mime_for(filename).ok()?}))}else{None}}).take(MAX_RESOURCE_IMAGES).collect())}
fn merge_image_resources(existing:Value,extra:Value)->Value{let mut items=existing.as_array().cloned().unwrap_or_default();let mut seen=items.iter().filter_map(|item|item.get("uri").and_then(Value::as_str).map(str::to_owned)).collect::<std::collections::HashSet<_>>();for item in extra.as_array().cloned().unwrap_or_default(){if let Some(uri)=item.get("uri").and_then(Value::as_str){if seen.insert(uri.to_owned()){items.push(item);}}}Value::Array(items)}
fn export_context_payload(state:&BridgeHttpState,args:&Value)->Result<Value,(i32,String)>{
let Some(context)=load_active_export_context(state)else{return Ok(json!({"active":false,"message":"공유된 내보내기 범위가 없습니다."}));};
let Some(entry_id)=context.get("entryId").and_then(Value::as_str).filter(|value|!value.trim().is_empty())else{return Ok(json!({"active":false,"message":"공유된 내보내기 범위가 없습니다."}));};
let entry=state.store.get_entry(entry_id).map_err(store_error)?.ok_or((-32004,"항목을 찾지 못했습니다.".to_owned()))?;
let submitted=context.get("submitted").and_then(Value::as_bool).unwrap_or(false);
let include_images=args.get("includeImages").and_then(Value::as_bool).unwrap_or(true);
let share=context.get("shareOptions").cloned().unwrap_or_else(||json!({}));
let share_user=share.get("shareUserResponse").and_then(Value::as_bool).unwrap_or(true);
let share_note=share.get("shareScratchNote").and_then(Value::as_bool).unwrap_or(true);
let share_images=share.get("shareQuestionImages").and_then(Value::as_bool).unwrap_or(true);
let share_source_pages=share.get("shareSourcePageImages").and_then(Value::as_bool).unwrap_or(false);
let numbers=context.get("questionNumbers").and_then(Value::as_array).cloned().unwrap_or_default();
let session_id=context.get("sessionId").and_then(Value::as_str).filter(|value|!value.trim().is_empty()).map(str::to_owned);
let session=if let Some(id)=session_id.as_deref(){load_exam_sessions(state)?.into_iter().find(|item|item.get("id").and_then(Value::as_str)==Some(id))}else{None};
let mut questions=Vec::new();
for raw in numbers{
let Some(number)=raw.as_str().filter(|value|!value.trim().is_empty())else{continue;};
let session_question=session.as_ref().and_then(|sess|sess.get("questions").and_then(Value::as_array).and_then(|items|items.iter().find(|item|item.get("questionNumber").and_then(Value::as_str).is_some_and(|value|normalize_question_number(value)==normalize_question_number(number))).cloned()));
let mut item=if entry.entry_kind=="wrong_answer"{json!({"entryId":entry.id,"questionNumber":number,"question":entry.question,"choices":[]})}else if let Some(question)=state.store.get_question(entry_id,number).map_err(store_error)?{json!({"entryId":question.entry.id,"questionNumber":question.question_number,"question":question.body,"choices":question.choices})}else{json!({"entryId":entry.id,"questionNumber":number,"question":Value::Null,"choices":[]})};
if let Some(sess)=session.as_ref(){
if let Some(q)=sess.get("questions").and_then(Value::as_array).and_then(|items|items.iter().find(|item|item.get("questionNumber").and_then(Value::as_str).is_some_and(|value|normalize_question_number(value)==normalize_question_number(number)))){
if let Some(passage)=q.get("passage"){item["passage"]=passage.clone();}
if let Some(segments)=q.get("contentSegments"){item["contentSegments"]=segments.clone();}
}
if let Some(response)=sess.get("responses").and_then(Value::as_array).and_then(|items|items.iter().find(|item|item.get("questionNumber").and_then(Value::as_str).is_some_and(|value|normalize_question_number(value)==normalize_question_number(number)))){
if share_user{item["userResponse"]=response.get("response").cloned().unwrap_or(Value::String(String::new()));}
if share_note{item["scratchNote"]=response.get("scratchNote").cloned().unwrap_or(Value::String(String::new()));}
}
}
if include_images{let mut images=Value::Array(Vec::new());if share_images{images=if entry.entry_kind=="wrong_answer"{image_resources_for_entry(state,&entry,Some(number))}else{image_resources(state,&entry,number)};}if share_source_pages{if let (Some(sess),Some(q))=(session.as_ref(),session_question.as_ref()){images=merge_image_resources(images,exam_image_resources(state,sess,q,false,true));}else if !entry.question_images.is_empty(){images=merge_image_resources(images,export_image_entries(state,&entry.id,&entry.question_images));}}if images.as_array().is_some_and(|items|!items.is_empty()){item["images"]=images;}}
questions.push(item);
}
Ok(json!({"active":true,"entryId":entry.id,"title":entry.title,"subject":entry.subject,"scope":context.get("scope"),"questionNumbers":context.get("questionNumbers"),"submitted":submitted,"answerProtection":if submitted{"released"}else{"active"},"sessionId":session_id,"updatedAt":context.get("updatedAt"),"questions":questions}))
}
...
fn resource_read(state:&BridgeHttpState,args:&Value)->Result<(Value,usize),(i32,String)>{let uri=args.get("uri").and_then(Value::as_str).ok_or((-32602,"uri가 필요합니다.".to_owned()))?;if let Some(value)=uri.strip_prefix("notebook-exam-image://session/"){let parts=value.split('/').collect::<Vec<_>>();if parts.len()!=3||parts.iter().any(|item|item.is_empty()||item.contains('\\')){return Err((-32602,"지원하지 않는 시험 이미지 URI입니다.".into()));}let sessions=load_exam_sessions(state)?;let session=sessions.iter().find(|item|item.get("id").and_then(Value::as_str)==Some(parts[0])).ok_or((-32004,"시험 세션을 찾지 못했습니다.".into()))?;let question=session.get("questions").and_then(Value::as_array).and_then(|items|items.iter().find(|item|item.get("questionNumber").and_then(Value::as_str)==Some(parts[1]))).ok_or((-32004,"시험 문항을 찾지 못했습니다.".into()))?;let listed=question.get("questionImages").and_then(Value::as_array).is_some_and(|items|items.iter().any(|item|item.as_str()==Some(parts[2])))||question.get("figures").and_then(Value::as_array).is_some_and(|items|items.iter().any(|item|item.get("image").and_then(Value::as_str)==Some(parts[2])))||question.get("sourcePageImages").and_then(Value::as_array).is_some_and(|items|items.iter().any(|item|item.as_str()==Some(parts[2])));if !listed||!namesafe_image(&state.images_path,parts[2]).map_err(store_error)?{return Err((-32004,"시험 이미지 리소스를 찾지 못했습니다.".into()));}let bytes=fs::read(state.images_path.join(parts[2])).map_err(|error|store_error(error.to_string()))?;let mime=mime_for(parts[2]).map_err(store_error)?;return Ok((json!({"contents":[{"uri":uri,"mimeType":mime,"blob":BASE64_STANDARD.encode(bytes)}]}),1));}let Some((entry_id,filename))=parse_resource_uri(uri)else{return Err((-32602,"지원하지 않는 이미지 URI입니다.".into()));};let entry=state.store.get_entry(entry_id).map_err(store_error)?.ok_or((-32004,"항목을 찾지 못했습니다.".to_owned()))?;let listed=entry.question_images.iter().any(|item|item==filename)||entry.figures.iter().any(|figure|figure.image.as_deref()==Some(filename));if !listed{return Err((-32004,"이미지 리소스를 찾지 못했습니다.".into()));}if !namesafe_image(&state.images_path,filename).map_err(store_error)?{return Err((-32004,"이미지 리소스를 찾지 못했습니다.".into()));}let path=state.images_path.join(filename);let bytes=fs::read(path).map_err(|error| store_error(error.to_string()))?;let mime=mime_for(filename).map_err(store_error)?;Ok((json!({"contents":[{"uri":uri,"mimeType":mime,"blob":BASE64_STANDARD.encode(bytes)}]}),1))}
...
mod tests {
    ...
    async fn actual_streamable_http_round_trip_enforces_protocol_and_is_read_only() {
        ...
        let tools=client.post(&url).bearer_auth("test-token").json(&request(Some(2),"tools/list",json!({}))).send().await.unwrap().json::<Value>().await.unwrap(); let tools=tools["result"]["tools"].as_array().unwrap(); assert_eq!(tools.len(),9); assert!(tools.iter().all(|tool|tool.get("inputSchema").is_some()&&tool["annotations"]["readOnlyHint"]==true));
