...
export default function SettingsAiPanel({ provider, status, statusLoading, statusError, keyInput, onKeyInputChange, onConfigChange, onStoreKey, onRemoveKey, onTestConnection }: SettingsAiPanelProps) {
  ...
  const commitModel = async () => {
    if (modelDraft === (provider.model ?? "")) { modelDirtyRef.current = false; return; }
    const saved = await onConfigChange({ model: modelDraft });
    if (saved !== false) modelDirtyRef.current = false;
  };
  const commitBaseUrl = async () => {
    if (baseUrlDraft === (provider.baseUrl ?? "")) { baseUrlDirtyRef.current = false; return; }
    const saved = await onConfigChange({ baseUrl: baseUrlDraft });
    if (saved !== false) baseUrlDirtyRef.current = false;
  };
  return <div className="ai-provider-settings">
    ...
    <label className="form-field"><span>모델</span><input value={modelDraft} disabled={!isTauri()} onChange={(event) => { modelDirtyRef.current = true; setModelDraft(event.target.value); }} onBlur={() => void commitModel()} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void commitModel(); } }} placeholder={selectedProvider === "openrouter" ? "예: openai/gpt-5.1-mini" : "예: gpt-5.1-mini"} /></label>
    ...
    <label className="form-field"><span>기본 URL (선택)</span><input value={baseUrlDraft} disabled={!isTauri()} onChange={(event) => { baseUrlDirtyRef.current = true; setBaseUrlDraft(event.target.value); }} onBlur={() => void commitBaseUrl()} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void commitBaseUrl(); } }} placeholder="https://api.example.com" /></label>
