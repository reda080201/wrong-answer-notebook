...
export function SettingsExamPanel({ preferences, onPatch }: { preferences: ExamPreferences; onPatch(patch: Partial<ExamPreferences>): void }) {
  return <div className="settings-pref-panel">
    ...
    <p className="settings-label">시험지 보기</p><div className="theme-options" role="group" aria-label="시험지 보기 방식"><button type="button" className={`theme-btn ${preferences.paperNavigation !== "two-up" ? "active" : ""}`} aria-pressed={preferences.paperNavigation !== "two-up"} onClick={() => onPatch({ paperNavigation: "vertical-pages" })}>세로 페이지</button><button type="button" className={`theme-btn ${preferences.paperNavigation === "two-up" ? "active" : ""}`} aria-pressed={preferences.paperNavigation === "two-up"} onClick={() => onPatch({ paperNavigation: "two-up" })}>2문항 펼침</button></div>
