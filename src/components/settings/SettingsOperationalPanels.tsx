...
export function SettingsExamPanel({ preferences, onPatch }: { preferences: ExamPreferences; onPatch(patch: Partial<ExamPreferences>): void }) {
  return <div className="settings-pref-panel">
    ...
    <p className="settings-label">시험지 표시 형식</p><div className="theme-options" role="group" aria-label="시험지 표시 형식"><button type="button" className={`theme-btn ${preferences.paperPresentation !== "two-question" ? "active" : ""}`} aria-pressed={preferences.paperPresentation !== "two-question"} onClick={() => onPatch({ paperPresentation: "a4" })}>실제 A4</button><button type="button" className={`theme-btn ${preferences.paperPresentation === "two-question" ? "active" : ""}`} aria-pressed={preferences.paperPresentation === "two-question"} onClick={() => onPatch({ paperPresentation: "two-question" })}>2문항 집중</button></div>
    {preferences.paperPresentation !== "two-question" && <><p className="settings-label">페이지 이동</p><div className="theme-options" role="group" aria-label="페이지 이동 방식"><button type="button" className="theme-btn" aria-pressed={preferences.paperNavigation !== "horizontal-pages"} onClick={() => onPatch({ paperNavigation: "vertical-pages" })}>세로 연속</button><button type="button" className="theme-btn" aria-pressed={preferences.paperNavigation === "horizontal-pages"} onClick={() => onPatch({ paperNavigation: "horizontal-pages" })}>가로 페이지</button></div></>}
