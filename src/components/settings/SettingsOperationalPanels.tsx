...
export function SettingsAdvancedPanel({ onReplayOnboarding }: { onReplayOnboarding?: () => void } = {}) { return <div className="advanced-settings-panel"><p>고급 설정은 진단 정보와 위험한 옵션을 분리해 두는 공간입니다.</p><p>백업, 복원, 무결성 검사, 이미지 정리는 데이터 관리 탭에서 실행하세요.</p>{onReplayOnboarding && <button type="button" className="btn-secondary" onClick={onReplayOnboarding}>시작 안내 다시 보기</button>}</div>; }
