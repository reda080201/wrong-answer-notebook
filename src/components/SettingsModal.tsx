          ...
          <section className="settings-modal-panel">
            ...
            {activeTab === "library" && (
              <div className="settings-pref-panel">
                ...
                <p className="settings-label">특강 본문 폭</p>
                <div className="theme-options" role="group" aria-label="특강 본문 폭">
                  {([['narrow', '좁게'], ['standard', '표준'], ['wide', '넓게'], ['full', '전체']] as const).map(([value, label]) => (
                    <button key={value} type="button" className={`theme-btn ${(settings.viewPreferences.lectureBodyWidth ?? "standard") === value ? "active" : ""}`} aria-pressed={(settings.viewPreferences.lectureBodyWidth ?? "standard") === value} onClick={() => void patchView({ lectureBodyWidth: value })}>{label}</button>
                  ))}
                </div>
