...
export default function SimilarQuestionLinksPanel({ entryId, block, entrySubject, items, onOpen, onChange }: Props) {
  ...
  const rerankWithGemini = async () => {
    ...
    try {
      ...
      setGeminiSuggestions(ranking.map((result) => { const base = byId.get(result.candidateId); return base ? { ...base, score: result.score ?? base.score, reasons: result.reasons ?? base.reasons, sharedConcepts: result.sharedConcepts ?? base.sharedConcepts, differences: result.differences ?? base.differences } : null; }).filter((item): item is NonNullable<typeof item> => Boolean(item)));
    } catch (error) { setGeminiError(error instanceof Error ? error.message : "Gemini 재정렬에 실패했습니다."); } finally { setBusy(false); }
  };
