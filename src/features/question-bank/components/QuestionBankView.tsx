...
export default function QuestionBankView({ entries, onOpenQuestion, preferences, onPreferencesChange, onRegisterPreferenceFlush, onPatchQuestionClassification, onRegisterScrollContainer, onStartReview, subjectFilter }: QuestionBankViewProps) {
  ...
  const filtered = useMemo(() => sortQuestionBankItems(filterQuestionBankItems(items, filters).filter((item) => !subjectFilter || item.subject === subjectFilter), sort), [items, filters, sort, subjectFilter]);
  const picked = useMemo(() => pickedIds.map((id) => items.find((item) => item.id === id)).filter((item): item is QuestionBankItem => Boolean(item)), [items, pickedIds]);
  useEffect(() => {
