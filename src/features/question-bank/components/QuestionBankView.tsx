...
export default function QuestionBankView({ entries, onOpenQuestion, preferences, onPreferencesChange, onRegisterPreferenceFlush, onPatchQuestionClassification, onRegisterScrollContainer, onStartReview }: QuestionBankViewProps) {
  ...
  return <section className="question-bank-view" aria-label="문제 은행">
    ...
    <div className="question-bank-actions">
      ...
      <label>보기 기준 <select value={viewMode} disabled={maintenanceBlocked} onChange={(event) => { const next = event.target.value as QuestionBankViewMode; setViewMode(next); savePreferences({ lastViewMode: next }); }}><option value="unit">단원별</option><option value="source">자료별</option><option value="recent">최근 학습</option><option value="important">중요 문제</option><option value="review">검토 필요</option></select></label>
    ...
    {picked.length > 0 && <div className="question-bank-picked" role="status">추출된 문항 {picked.map((item) => `${item.entryTitle} ${item.questionNumber}번`).join(" · ")}<button type="button" className="btn-primary btn-sm" onClick={() => onStartReview?.(picked)}>이 {picked.length}개 복습 시작</button><button type="button" className="btn-secondary btn-sm" onClick={() => setPicked(selectQuestionBankItems(filtered, Math.min(10, filtered.length), `${Date.now()}-${Math.random()}`))}>다시 추출</button><button type="button" className="btn-secondary btn-sm" onClick={() => setPicked([])}>선택 지우기</button></div>}
