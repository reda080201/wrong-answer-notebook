...
export default function QuestionBankView({ entries, onOpenQuestion, preferences, onPreferencesChange, onRegisterPreferenceFlush, onPatchQuestionClassification, onRegisterScrollContainer, onStartReview }: QuestionBankViewProps) {
  ...
  return <section className="question-bank-view" aria-label="문제 은행">
    ...
    {picked.length > 0 && <div className="question-bank-picked" role="status">추출된 문항 {picked.map((item) => `${item.entryTitle} ${item.questionNumber}번`).join(" · ")}<button type="button" className="btn-primary btn-sm" onClick={() => onStartReview?.(picked)}>이 10개 복습 시작</button><button type="button" className="btn-secondary btn-sm" onClick={() => setPicked(selectQuestionBankItems(filtered, Math.min(10, filtered.length), `${Date.now()}-${Math.random()}`))}>다시 추출</button><button type="button" className="btn-secondary btn-sm" onClick={() => setPicked([])}>선택 지우기</button></div>}
