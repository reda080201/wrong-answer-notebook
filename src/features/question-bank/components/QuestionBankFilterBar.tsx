...
export default function QuestionBankFilterBar({ items, filters, onChange, onReset, disabled = false }: QuestionBankFilterBarProps) {
  ...
  return <section className="question-bank-filters" aria-label="문제 은행 필터">
    <div className="question-bank-filter-search"><SearchField value={filters.search} onChange={(search) => onChange({ search })} placeholder="문제, 단원, 개념, 출처 검색" ariaLabel="문제 은행 검색" /><button type="button" className="btn-secondary" disabled={disabled} onClick={onReset}>초기화</button></div>
