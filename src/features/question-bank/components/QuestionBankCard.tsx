...
export default function QuestionBankCard({ item, onOpen, onInspect }: QuestionBankCardProps) {
  ...
  return <article className="question-bank-card">
    <button type="button" className="question-bank-card__main" onClick={() => onOpen(item)} aria-label={`${item.entryTitle} ${item.questionNumber}번 열기`}>
      <header><strong><b>{item.questionNumber}번</b> {item.entryTitle}</strong><span>{PROBLEM_SOURCE_LABELS[item.source.type]}</span></header>
      <div className="question-bank-card__chips"><span>{item.subject}</span>{classification.unit && <span>{classification.unit}</span>}</div>
    ...
    <button type="button" className="question-bank-card__detail" aria-label={`${item.questionNumber}번 검사 패널 열기`} onClick={() => onInspect(item)}>검사</button>
  </article>;
}
