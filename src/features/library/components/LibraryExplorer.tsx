...
import type { LibraryFolder, LibraryNavigationPreference, LibraryPreferences, WrongAnswerEntry } from "../../../types";
...
type ResourceGroup = "all" | "특강" | "문제" | "기출" | "N제" | "모의고사" | "미분류";
...
function groupForSection(section: LibraryNavigationPreference["section"]): ResourceGroup {
  if (section === "lectures") return "특강";
  if (section === "problems") return "문제";
  return "all";
}

function sectionForGroup(group: ResourceGroup): LibraryNavigationPreference["section"] {
  if (group === "특강") return "lectures";
  if (group === "문제") return "problems";
  return "all";
}

export default function LibraryExplorer({ folders, entries, preferences, navigation, onNavigationChange, onOpenEntry, onCreateFolder, onRenameFolder, onMoveFolder, onMoveEntries, onDeleteFolder }: LibraryExplorerProps) {
  ...
  const [resourceGroup, setResourceGroup] = useState<ResourceGroup>(() => groupForSection(navigation?.section)); const [search, setSearch] = useState(""); const [sort, setSort] = useState<SortKey>("updated");
  ...
  const updateNavigation = useCallback((next: { subject?: string | null; course?: string | null; unit?: string | null; group?: ResourceGroup }) => {
    ...
    onNavigationChange?.({ subject: nextSubject ?? undefined, course: nextCourse ?? undefined, unit: nextUnit ?? undefined, section: sectionForGroup(nextGroup) });
  ...
  useEffect(() => {
    ...
    setSubject(navigation.subject ?? null); setCourse(navigation.course ?? null); setUnit(navigation.unit ?? null); setResourceGroup(groupForSection(navigation.section));
  }, [navigation?.course, navigation?.section, navigation?.subject, navigation?.unit]);
