...
import { FileText, FolderInput, MoreHorizontal, Star } from "lucide-react";
...
import { compareLibraryClassifications, getLibraryResourceGroup, normalizeLibraryPreferences, projectLibraryResource, resourceTypeLabel } from "../../../utils/libraryClassification";
...
interface LibraryRowContextValue { displayMode: "standard" | "file"; folders: LibraryFolder[]; onFavorite?(entry: WrongAnswerEntry): Promise<void>; onClassify?(entryId: string): void; onMoveToFolder?(entryId: string, folderId?: string): void; }
const LibraryRowContext = createContext<LibraryRowContextValue>({ displayMode: "standard", folders: [] });
...
function EntryRow({ entry, separateMockExams, onOpen, selected, onSelect }: { entry: WrongAnswerEntry; separateMockExams: boolean; onOpen(): void; selected?: boolean; onSelect?(): void }) {
  ...
  const context = useContext(LibraryRowContext);
  const [favoriteSaving, setFavoriteSaving] = useState(false);
  const [favoriteError, setFavoriteError] = useState(false);
  const toggleFavorite = async (event: MouseEvent<HTMLButtonElement>) => { event.stopPropagation(); if (!context.onFavorite || favoriteSaving) return; setFavoriteSaving(true); setFavoriteError(false); try { await context.onFavorite(entry); } catch { setFavoriteError(true); } finally { setFavoriteSaving(false); } };
  if (context.displayMode === "file") return <div className={`library-resource-row library-resource-row--file${selected ? " is-selected" : ""}`} draggable onDragStart={(event) => event.dataTransfer.setData("application/x-entry-ids", entry.id)}><input type="checkbox" aria-label={`${entry.title || "제목 없음"} 선택`} checked={selected === true} onChange={onSelect} onClick={(event) => event.stopPropagation()} /><FileText className="library-file-icon" aria-hidden="true" size={18} /><button type="button" className="library-resource-open library-file-open" onClick={onOpen} title={entry.title || "제목 없음"}><span className="library-resource-name">{entry.title || "제목 없음"}</span><span className="library-resource-subject">{projection.classification.subject}</span><span className="library-resource-type">{getLibraryResourceGroup(entry, { separateMockExams })}</span><span className="library-resource-count">{countQuestions(entry)}문항</span><time dateTime={entry.updatedAt}>{new Date(entry.updatedAt).toLocaleDateString("ko-KR")}</time></button><button type="button" className="library-file-favorite" aria-pressed={entry.libraryFavorite === true} aria-label={entry.libraryFavorite === true ? "즐겨찾기에서 제거" : "즐겨찾기에 추가"} title={entry.libraryFavorite === true ? "즐겨찾기에서 제거" : "즐겨찾기에 추가"} disabled={favoriteSaving} onClick={(event) => void toggleFavorite(event)}><Star size={17} fill={entry.libraryFavorite === true ? "currentColor" : "none"} /></button><Menu label={<MoreHorizontal size={18} />} triggerAriaLabel={`${entry.title || "제목 없음"} 더보기`} className="library-file-menu" stopPropagation><button type="button" onClick={onOpen}>열기</button>{context.onClassify ? <button type="button" onClick={() => context.onClassify?.(entry.id)}>분류 수정</button> : null}{context.folders.length ? <><span className="menu-section-label">폴더로 이동</span>{context.folders.map((folder) => <button key={folder.id} type="button" onClick={() => context.onMoveToFolder?.(entry.id, folder.id)}><FolderInput size={14} />{folder.name}</button>)}</> : null}</Menu>{favoriteError ? <span className="library-file-action-error" role="status">저장하지 못했습니다. 다시 시도해 주세요.</span> : null}</div>;
...
export default function LibraryExplorer({ folders, entries, preferences, navigation, onNavigationChange, onOpenEntry, onCreateFolder, onRenameFolder, onMoveFolder, onMoveEntries, onUpdateEntries, onDeleteFolder }: LibraryExplorerProps) {
  const libraryPreferences = normalizeLibraryPreferences(preferences ?? DEFAULT_LIBRARY_PREFERENCES);
  ...
  return <LibraryRowContext.Provider value={{ displayMode: libraryPreferences.displayMode ?? "standard", folders, onFavorite: onUpdateEntries ? async (entry) => { await onUpdateEntries([entry.id], { libraryFavorite: !entry.libraryFavorite }); } : undefined, onClassify: (entryId) => { setSelectedIds(new Set([entryId])); setClassificationDraft(entries.find((entry) => entry.id === entryId)?.resourceClassification ?? {}); setClearFields(new Set()); setClassificationOpen(true); }, onMoveToFolder: onMoveEntries ? (entryId, folderId) => onMoveEntries([entryId], folderId) : undefined }}><section className="library-explorer" aria-label="학습 자료 보관함">
  ...
  </section></LibraryRowContext.Provider>;
}
