...
describe("structured import draft canonical helpers", () => {
  it("merges reviewed compatibility text without moving figure segments", () => {
    const result = mergeCompatibilityTextIntoStructuredQuestions([question], "1. after\n① new");
    expect(result.error).toBeUndefined();
    expect(result.questions?.[0].questionText).toBe("after");
    expect(result.questions?.[0].choices).toEqual(["① new"]);
    expect(result.questions?.[0].contentSegments.map((segment) => segment.id)).toEqual(["text-1", "figure-slot"]);
  });

  it("rejects changed or duplicate question numbers", () => {
    expect(mergeCompatibilityTextIntoStructuredQuestions([question], "2. after").error).toMatch(/추가하거나 삭제/);
    expect(mergeCompatibilityTextIntoStructuredQuestions([question], "1. after\n\n1. duplicate").error).toMatch(/중복/);
  });
  ...
  it("round-trips a reviewed canonical question into projection and exam creation", () => {
    const merged = mergeCompatibilityTextIntoStructuredQuestions([question], "1. after\n① new").questions!;
