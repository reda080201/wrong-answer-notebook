...
const entry = {
  ...
  questions: [{ questionNumber: "1", questionText: "QUESTION SECRET", choices: ["CHOICE SECRET"], contentSegments: [], conditions: [], equations: [], figureIds: [] }],
...
describe("ChatGptSharePanel copied prompt", () => {
  ...
  it("copies only selected projected fields without syncing MCP or opening ChatGPT", async () => {
    ...
    for (const secret of ["QUESTION SECRET", "CHOICE SECRET", "RESPONSE SECRET", "NOTE SECRET", "ANSWER SECRET", "EXPLANATION SECRET"]) expect(copied).not.toContain(secret);
