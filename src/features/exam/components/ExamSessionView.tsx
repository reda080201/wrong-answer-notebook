...
export default function ExamSessionView({ session, onChange, onUpdateSession, onSubmit, onSubmittingChange, onAskGpt, examPreferences, onOpenSettings, chatGptPreferences, onChatGptPreferencesChange, onSyncChatGptContext, onOpenChatGptSettings, onCheckLocalMcp, remoteMcpConfigured, onClose, onStartReview }: ExamSessionViewProps) {
  ...
  return <section className="exam-session-view" aria-label="문제 풀기 응시">
    ...
    <Dialog open={submitOpen} onClose={() => setSubmitOpen(false)} backdropClassName="exam-dialog-backdrop" className="exam-dialog" title="시험을 제출할까요?" closeDisabled={submitting} busy={submitting} footer={<div className="dialog-footer-actions"><button type="button" className="btn-secondary" onClick={() => setSubmitOpen(false)} disabled={submitting}>계속 풀기</button><button type="button" className="btn-primary" onClick={() => void submit()} disabled={submitting}>{submitting ? "제출 중…" : "제출하고 채점"}</button></div>}><p>전체 {session.questions.length}문항 · 응답 {session.questions.length - unanswered.length}문항 · 미응답 {unanswered.length}문항 · 검토 표시 {marked.length}문항</p>{warnUnanswered && unanswered.length > 0 && <p>미응답: {unanswered.join(", ")}번</p>}{submitError && <p className="form-error" role="alert">{submitError}</p>}</Dialog>
  </section>;
}
