...
export function createSessionFromGeneratedExam(exam: GeneratedExam, now = new Date(), options: { mode?: ExamMode; timeLimitMinutes?: number; showTimer?: boolean; answerSheetOpen?: boolean; answerSheetLayout?: "auto" | "vertical" | "horizontal" } = {}): ExamSession {
  ...
  return { id: uuidv4(), entryId: `generated:${exam.id}`, title: exam.title, subject: exam.subject, status: "in_progress", mode, timeLimitMinutes, deadlineAt: timeLimitMinutes ? new Date(now.getTime() + timeLimitMinutes * 60_000).toISOString() : undefined, showTimer: mode === "real" ? options.showTimer !== false : undefined, answerSheetOpen: mode === "real" ? options.answerSheetOpen !== false : undefined, answerSheetLayout: mode === "real" ? options.answerSheetLayout ?? "auto" : undefined, questions: exam.questions.map((question) => { const source = question.source ?? migrateQuestionSource(question, []).source; return { ...structuredClone(question.snapshot), generatedExamId: exam.id, sourceEntryId: source.sourceEntryId, sourceQuestionNumber: source.sourceQuestionNumber, generatedQuestionPosition: question.position }; }), responses: [], currentQuestionIndex: 0, startedAt, updatedAt: startedAt };
}
