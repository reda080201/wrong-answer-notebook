...
export default function ExamBuilderWizard({ entries, selectedEntryIds = [], onSave, onStart, onClose }: Props) {
  ...
  const save = async (start: boolean) => { if (!generated) return; setBusy(true); try { const ready = { ...generated, status: "ready" as const, updatedAt: new Date().toISOString() }; await onSave(ready); if (start) await onStart(ready); else onClose(); } finally { setBusy(false); } };
  return <div className="modal-backdrop" onClick={onClose}><section className="modal-card exam-builder-modal" role="dialog" aria-modal="true" aria-label="모의고사 만들기" onClick={(event) => event.stopPropagation()}>
