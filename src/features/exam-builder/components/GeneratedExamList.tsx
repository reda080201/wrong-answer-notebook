      ...
      {exams.length ? (
        <div>{[...exams].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((exam) => (
          <article key={exam.id}>
            ...
            <div><button type="button" onClick={() => onOpen(exam)}>{exam.status === "ready" ? "풀기" : "검토"}</button>{onPrint && <button type="button" className="btn-secondary" onClick={() => onPrint(exam)}>PDF 만들기</button>}<button type="button" className="btn-secondary" onClick={() => onDelete(exam.id)}>삭제</button></div>
          </article>
        ))}</div>
