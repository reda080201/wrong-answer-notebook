...
export default function LearningHubView({ entries, onOpenSource, onUpdateBlock, onDuplicateBlock, onDeleteBlock, onOpenCandidateReview, questionBankItems = [], highlightedBlock = null, aiProviderStatus, onOpenAiSettings }: LearningHubViewProps) {
  ...
  return <section className="learning-hub" aria-label="학습 허브">
    ...
    {filtered.length ? <div className="learning-hub-outline"><nav className="learning-hub-outline__index" aria-label="학습 내용 목차"><h3>목차</h3>{filtered.map((item, index) => { const key = `${item.sourceEntryId}:${item.block.id}`; const selected = selectedBlockKey === key; return <button type="button" key={key} className={selected ? "is-selected" : ""} aria-current={selected ? "true" : undefined} onClick={() => setSelectedBlockKey(key)}><span>{String(index + 1).padStart(2, "0")}. {item.block.title || "학습 내용"}</span><small>{item.sourceSubject} · {item.block.unit ?? "단원 미분류"}</small></button>; })}</nav><div className="learning-hub-outline__detail">{(() => { const selected = filtered.find((item) => `${item.sourceEntryId}:${item.block.id}` === selectedBlockKey) ?? filtered[0]; return <LearningBlockCard key={`${selected.sourceEntryId}:${selected.block.id}`} item={selected} highlighted={highlightedBlock?.entryId === selected.sourceEntryId && highlightedBlock.blockId === selected.block.id} onOpenSource={onOpenSource} onUpdateBlock={onUpdateBlock} onDuplicateBlock={onDuplicateBlock} onDeleteBlock={onDeleteBlock} questionBankItems={questionBankItems} aiProviderStatus={aiProviderStatus} onOpenAiSettings={onOpenAiSettings} />; })()}</div></div> : <div className="detail-panel empty-state"><p>조건에 맞는 학습 카드가 없습니다.</p></div>}
  </section>;
}
